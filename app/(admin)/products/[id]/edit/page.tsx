...
export default function EditProductPage() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                setTechStack(data.tech_stack?.length ? data.tech_stack : [""]);
                ...
                if (data.price) {
                    ...
                    const formatted = new Intl.NumberFormat("id-ID").format(Number(data.price));
    ...
    const handleRemoveImage = (index: number) => {
        ...
        // Check if it's an existing image (starts with http) or new preview (blob)
        if (imageUrl.startsWith('http')) {
            // It's an existing image, add to deleted images
    ...
    const handleSubmit = async (formData: FormData) => {
        ...
        startTransition(async () => {
            try {
                ...
                const updatedProduct = await updateProduct(parseInt(productId), formData);

                console.log('✅ Product updated successfully:', updatedProduct);

                // Send WebSocket notification
                if (updatedProduct && websocketService.isConnected()) {
                    console.log('📢 Sending WebSocket notification for updated product');
                    websocketService.notifyProductUpdated(updatedProduct);
                } else {
    ...
    if (loading) {
        return (
            <div className="max-w-4xl mx-auto my-12 p-10 border rounded-3xl" style={{ backgroundColor: 'var(--card-background)', borderColor: 'var(--card-border)' }}>
    ...
    return (
        <div className="max-w-4xl mx-auto my-12 p-10 border rounded-3xl" style={{ backgroundColor: 'var(--card-background)', borderColor: 'var(--card-border)', boxShadow: 'var(--card-shadow)' }} suppressHydrationWarning>
            <header className="mb-10 border-b pb-6" style={{ borderColor: 'var(--border-muted)' }}>
                <h1 className="text-4xl font-black tracking-tight" style={{ color: 'var(--text-primary)' }}>Edit Produk</h1>
                <p className="mt-2" style={{ color: 'var(--text-secondary)' }}>Perbarui detail produk digital Anda di bawah ini.</p>
            </header>
            ...
            {success && (
                <div className="mb-6 p-4 rounded-xl" style={{ backgroundColor: 'var(--success)20', borderColor: 'var(--success)' }}>
                    <p className="font-medium" style={{ color: 'var(--success)' }}>✅ Produk berhasil diperbarui! Mengalihkan...</p>
                </div>
            )}
                    ...
                    <div className="flex flex-wrap gap-4">
                        {previews.map((url, i) => (
                            <div key={i} className="relative group">
                                <div className="relative w-32 h-32 rounded-2xl overflow-hidden border-2 shadow-md" style={{ borderColor: 'var(--border-muted)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            type="url"
                            disabled={isPending}
                            defaultValue={product?.link_program || ""}
                            className="w-full h-12 px-4 border-2 rounded-xl font-medium outline-none transition-all disabled:opacity-50"
                            style={{
                                borderColor: 'var(--border-secondary)',
                                color: 'var(--text-primary)',
